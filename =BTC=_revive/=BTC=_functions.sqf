...
BTC_set_gear =
{
	...
	if (count (_gear select 11) > 0) then {{if (_x == "Binocular" || _x == "Laserdesignator" || _x == "Rangefinder" ) then {_unit addWeapon _x;};if (_x != "" && _x != "Binocular") then {_unit addItem _x;_unit assignItem _x;sleep 0.01;};} foreach (_gear select 11);};
	removeBackPack _unit;
	...
	_watch = 0;
	while {loadUniform _unit < 0.97} do {_unit addItem "itemWatch";_watch = _watch + 1;sleep 0.0001;};
	...
	while {loadVest _unit < 0.97} do {_unit addItem "itemWatch";_watch = _watch + 1;sleep 0.0001;};
